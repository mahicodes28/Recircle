  ...
  return (
    <div className="searchBox h-10 sm:h-12 bg-white rounded-4xl !text-black relative !py-1 md:py-2 xl:py-2 !px-4 flex items-center justify-center w-full">
      ...
      {showResults && filteredProducts.length > 0 && (
        <div className="search-results w-[50vw] h-[50vh] backdrop-blur-sm bg-white/50 rounded-lg overflow-y-scroll absolute top-15 left-0 flex flex-col lg:flex-row xl:flex-row flex-wrap z-999 ">
          {filteredProducts.map((product) => (
            <div
              key={product._id}
              className="mb-2  scale-50 xl:scale-75  last:mb-0 w-[16vw]"
            >
              <ProductItem
                product={product}
