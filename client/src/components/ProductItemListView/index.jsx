    ...
    <>
      <div className="productItem bg-red-200 scale-100 !h-[20vw] flex !shadow-lg items-center !w-[97%] transition-all !mt-5 !rounded-lg">
        <div className="imgWrapper relative !w-[40%] !h-[20vw]">
          <Link to={product.link || `/product/${product._id}`} className='link w-[100%] h-full'>
            <img src={product.image} className='!w-full !rounded-lg !shadow-md overflow-hidden !h-[20vw] object-fit' alt={product.name} />
          </Link>
        ...
        <div className="info !h-[full] !px-5 text-left w-full flex flex-col justify-center items-start">
          <h6 className='Brand w-full text-left text-2xl transition-all'>
            <Link className='link' to={product.link || `/product/${product._id}`}>{product.brand}</Link>
          </h6>
          <h3 className='Title w-full text-left text-4xl transition-all'>
            <Link className='link' to={product.link || `/product/${product._id}`}>{product.name}</Link>
          </h3>
          <h3 className='Title w-full text-left text-sm text-zinc-600 transition-all'>
            <span>{product.description}</span>
          </h3>
          <Stack spacing={1}>
            <Rating className='!w-[10%]' name="half-rating-read" defaultValue={product.rating} precision={0.5} readOnly />
          </Stack>
          ...
          <Button className='link !bg-blue-600 !text-white !mt-4 !gap-2'>
